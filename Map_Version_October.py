...
@app.route('/')
def index():
    # Run your folium map generation code here
    file_path = 'data.xlsx'
    data = pd.read_excel(file_path)
    
    # Map generation (similar to your current code)
    m = folium.Map(location=[data['Latitude'].mean(), data['Longitude'].mean()], zoom_start=6)
    
    # Save to an HTML file
    m.save('templates/out_of_school_map.html')
    return render_template('out_of_school_map.html')
